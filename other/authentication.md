...
即`stoken_v2`和`stoken`。

`stoken_v2`和`stoken`并不总是使用相同的值，多用于在米游社内的操作。其被命名为“SToken（V1）”与“SToken（V2）”。

`stoken_v2`必须与[`mid`](#mihoyo-id)一起使用，`stoken`必须与[`stuid`](#account-id)一起使用。
